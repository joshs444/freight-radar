...
const ALIASES = {
  ...
  dest_port: ['dest_port', 'destination', 'destination_port', 'to', 'to_port', 'discharge_port', 'pod', 'dest port'],
  ...
  annual_value_usd: ['annual_value_usd', 'value', 'annual_value', 'value_usd', 'usd', 'annual value'],
...
const num = (s) => { const v = parseFloat(String(s).replace(/[, $]/g, '')); return Number.isFinite(v) ? v : 0; };

export function lanesFromCSV(text) {
