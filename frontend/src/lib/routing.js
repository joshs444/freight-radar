...
const CORRIDOR_RAW = [
  [['East Asia', 'North Europe'], ['Malacca Strait', 'Bab el-Mandeb Strait', 'Suez Canal', 'Gibraltar Strait', 'Dover Strait']],
  [['Southeast Asia', 'North Europe'], ['Malacca Strait', 'Bab el-Mandeb Strait', 'Suez Canal', 'Gibraltar Strait', 'Dover Strait']],
  [['South Asia', 'North Europe'], ['Bab el-Mandeb Strait', 'Suez Canal', 'Gibraltar Strait', 'Dover Strait']],
  [['East Asia', 'Mediterranean'], ['Malacca Strait', 'Bab el-Mandeb Strait', 'Suez Canal']],
  [['Southeast Asia', 'Mediterranean'], ['Malacca Strait', 'Bab el-Mandeb Strait', 'Suez Canal']],
  ...
  [['Gulf', 'North Europe'], ['Bab el-Mandeb Strait', 'Suez Canal', 'Gibraltar Strait', 'Dover Strait']],
  [['Gulf', 'Mediterranean'], ['Bab el-Mandeb Strait', 'Suez Canal']],
  [['Black Sea', 'North Europe'], ['Gibraltar Strait', 'Dover Strait']],
...
const normLocode = (s) => String(s || '').toUpperCase().replace(/[^A-Z0-9]/g, '');
...
export function makeResolver(lookup) {
  ...
  const byId = new Map(), byLoc = new Map(), byName = new Map();
  for (const r of lookup?.rows || []) {
    const rec = {
      portid: r[idx.portid], name: r[idx.name], locode: r[idx.locode],
      continent: r[idx.continent], lat: r[idx.lat], lon: r[idx.lon],
    };
    byId.set(rec.portid, rec);
    if (rec.locode) { const k = normLocode(rec.locode); if (!byLoc.has(k)) byLoc.set(k, rec); }
    const n = normName(rec.name); if (n && !byName.has(n)) byName.set(n, rec);
  }
  const resolve = (token) => {
    if (!token) return [null, null];
    if (byId.has(token)) return [byId.get(token), 'portid'];
    const lc = normLocode(token); if (byLoc.has(lc)) return [byLoc.get(lc), 'locode'];
    const nm = normName(token); if (byName.has(nm)) return [byName.get(nm), 'name'];
    return [null, null];
  };
...
export function routeLane(lane, resolver) {
  ...
  let oRec = null, dRec = null, oMatch = null, dMatch = null, regionDerived = false;
  if (resolver) {
    ...
    oRegion = or; dRegion = dr; oRec = orec; dRec = drec; oMatch = om; dMatch = dm;
  }
  const cps = new Set();
  for (const region of [oRegion, dRegion]) (GATEWAY[region] || []).forEach((x) => cps.add(x));
  for (const port of [lane.origin_port, lane.dest_port]) (PORT_CHOKEPOINTS[port] || []).forEach((x) => cps.add(x));
