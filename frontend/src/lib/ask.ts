...
// the payload Ask reads — sidecars may be absent, so every access is optional
type AskData = AppData | null | undefined;
...
// an entity reference an answer can fly to / highlight
interface EntityRef {
  portid: string;
  name: string;
}

// the rendered answer the engine returns
interface Answer {
  text: string;
  facts: Fact[];
  cites: string[];
...
// a single record in the merged sidecar index (one chokepoint / flagged port / port)
interface IndexEntity {
  portid: string;
  name: string;
  type: string;
  snap?: SnapshotChokepoint;
  flag?: Flag;
  contrib?: StressContributor;
...
// the lookup the index exposes
interface AskIndex {
  byId: Record<string, IndexEntity>;
  nameToId: Record<string, string>;
}

// findEntity returns an index record plus the matched-name length used to break ties
interface MatchedEntity extends IndexEntity {
  _len: number;
}
...
const perDay = (v: number | null | undefined): string => (v == null ? '—' : `${Math.round(v)}/day`);

// a grounded fact: a raw value lifted straight from `src`
const F = (v: FactValue, src: string): Fact => ({ v, src });
...
export function buildIndex(data: AskData): AskIndex {
  const byId: Record<string, IndexEntity> = {};
  const get = (pid: string, name: string, type: string): IndexEntity =>
    (byId[pid] = byId[pid] || { portid: pid, name, type });

  (data?.snapshot?.chokepoints || []).forEach((c: SnapshotChokepoint) => {
    get(c.portid, c.name, 'chokepoint').snap = c;
  });
  (data?.flags || []).forEach((f: Flag) => {
    const r = get(f.portid, f.entity, f.kind?.startsWith('chokepoint') ? 'chokepoint' : 'port');
    if (f.lifecycle !== 'resolved') r.flag = f;
  });
  (data?.stress?.contributors || []).forEach((c: StressContributor) => {
    if (byId[c.portid]) byId[c.portid].contrib = c;
  });
  const series: Record<string, TimeseriesSeries> = data?.timeseries?.series || {};
  ...
  (data?.snapshot?.ports || []).forEach((p: SnapshotPort) => {
    const n = norm(p.name);
    if (n && !(n in nameToId)) {
      nameToId[n] = p.portid;
...
function findEntity(q: string, index: AskIndex): IndexEntity | null {
  const qn = ` ${norm(q)} `;
...
// --- answer builders -------------------------------------------------------
function summarize(data: AskData): Answer {
  const s = data?.stress;
  const cites: string[] = ['stress.json'];
  const facts: Fact[] = [];
  let text = '';
  if (s?.available) {
    facts.push(
...
function entityStatus(e: IndexEntity): Answer {
  const cites: string[] = [];
  const facts: Fact[] = [];
  let text = `**${e.name}**`;
  const f = e.flag;
...
function biggestRisk(data: AskData): Answer {
  const active = (data?.flags || []).filter((f: Flag) => f.lifecycle !== 'resolved');
  if (!active.length)
    return answer({
...
function trending(data: AskData): Answer | null {
  const s = data?.stress;
...
function exposure(e: IndexEntity | null, data: AskData): Answer | null {
  const ex = data?.exposure;
  // entity-specific exposure
  if (e?.flag?.business?.lane_count) {
    const b = e.flag.business;
    const tot: Partial<CostBand> =
      b.total_cost_of_disruption_usd || b.carrying_cost_of_delay_usd || {};
    const facts = [
  ...
  const tot: Partial<CostBand> =
    ex.total_cost_of_disruption_usd || ex.carrying_cost_of_delay_usd || {};
  const facts = [
...
function marketAnswer(data: AskData): Answer | null {
...
function weekAnswer(data: AskData): Answer | null {
  const b = data?.brief;
  const cites: string[] = [];
  const facts: Fact[] = [];
  let text = '';
...
function listDisrupted(data: AskData): Answer {
  const active = (data?.flags || [])
    .filter((f: Flag) => f.lifecycle !== 'resolved')
...
function worldAnswer(data: AskData): Answer | null {
  const w = data?.world;
  if (!w?.available || !w.metrics?.length) return null;
  const facts: Fact[] = [];
  const get = (k: string) => w.metrics.find((m) => m.key === k);
  const parts: string[] = [];
...
function gatunAnswer(data: AskData): Answer | null {
  const g = data?.gatun;
...
function stressExplain(data: AskData): Answer | null {
  const s = data?.stress;
  if (!s?.available) return null;
  const facts = [
    F(s.index, 'stress.json'),
...
function hazardAnswer(e: IndexEntity | null, data: AskData): Answer {
...
// --- intent routing --------------------------------------------------------
export function ask(q: string, data: AskData, prebuiltIndex?: AskIndex | null): Answer {
  const index = prebuiltIndex || buildIndex(data);
  const s = norm(q);
  if (!s) return HELP;
  const has = (...ws: string[]): boolean => ws.some((w) => s.includes(w));
