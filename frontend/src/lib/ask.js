...
const ALIASES = {
  hormuz: 'strait of hormuz', 'persian gulf': 'strait of hormuz',
  suez: 'suez canal', panama: 'panama canal', malacca: 'malacca strait',
  taiwan: 'taiwan strait', bosphorus: 'bosporus strait', bosporus: 'bosporus strait',
  istanbul: 'bosporus strait', gibraltar: 'gibraltar strait', kerch: 'kerch strait',
  oresund: 'oresund strait', 'bab el mandeb': 'bab el-mandeb strait',
  'bab-el-mandeb': 'bab el-mandeb strait', mandeb: 'bab el-mandeb strait',
  'red sea': 'bab el-mandeb strait', dover: 'dover strait', korea: 'korea strait',
  'cape of good hope': 'cape of good hope', cape: 'cape of good hope',
  shanghai: 'shanghai (pudong)', pudong: 'shanghai (pudong)',
};
...
export function buildIndex(data) {
  const byId = {};
  const get = (pid, name, type) =>
    (byId[pid] = byId[pid] || { portid: pid, name, type });
  ...
  Object.keys(byId).forEach((pid) => { if (series[pid]) byId[pid].series = series[pid]; });
  ...
  const nameToId = {};
  Object.values(byId).forEach((r) => { nameToId[norm(r.name)] = r.portid; });
  (data?.snapshot?.ports || []).forEach((p) => {
    const n = norm(p.name);
    if (n && !(n in nameToId)) { nameToId[n] = p.portid; get(p.portid, p.name, 'port').port = p; }
  });
...
function summarize(data) {
  ...
  let text = '';
  if (s?.available) {
    facts.push(F(s.index, 'stress.json'), F(s.wow_delta, 'stress.json'),
      F(s.chokepoints_disrupted, 'stress.json'), F(s.chokepoints_total, 'stress.json'));
    const mv = s.wow_direction === 'up' ? `up ${Math.abs(s.wow_delta)}`
      : s.wow_direction === 'down' ? `down ${Math.abs(s.wow_delta)}` : 'flat';
    text += `Global ocean-freight stress is **${s.index}/100 (${s.label})**, ${mv} week-over-week. `
    ...
    if (top) {
      ...
      text += ` The biggest driver is **${top.name}**, running ${perDay(top.now)} vs a normal of `
...
function entityStatus(e, data) {
  ...
  if (f) {
    cites.push('flags.json');
    facts.push(F(f.value, 'flags.json'), F(f.baseline, 'flags.json'),
      F(f.pct_change, 'flags.json'), F(f.severity, 'flags.json'));
    const kind = f.kind.replaceAll('_', ' ');
    text += ` is flagged — **${kind}** (severity ${f.severity}). It ran **${perDay(f.value)}** `
    ...
    cites.push('stress.json');
    facts.push(F(e.contrib.now, 'stress.json'), F(e.contrib.normal, 'stress.json'), F(e.contrib.pct_vs_normal, 'stress.json'));
    text += ` has no active disruption flag, but is currently at ${perDay(e.contrib.now)} `
...
function biggestRisk(data) {
  ...
  if (!active.length) return answer({ text: 'No active disruptions right now.', cites: ['flags.json'], kind: 'risk' });
  const top = active.slice().sort((a, b) => b.severity - a.severity)[0];
  const facts = [F(top.severity, 'flags.json'), F(top.value, 'flags.json'),
    F(top.baseline, 'flags.json'), F(top.pct_change, 'flags.json')];
  const text = `The biggest risk is **${top.entity}** — ${top.kind.replaceAll('_', ' ')}, severity `
    + `**${top.severity}**, running ${perDay(top.value)} vs ${perDay(top.baseline)} (${pct(top.pct_change)}).`;
  return answer({ text, facts, cites: ['flags.json'], entity: { portid: top.portid, name: top.entity }, kind: 'risk',
...
function trending(data) {
  ...
  const text = (parts.length ? parts.join(', and ') + ` over the last ${s.fastest_deteriorating?.days || 14} days. ` : '')
...
function marketAnswer(data) {
  ...
  want.forEach((k) => {
    ...
    if (m && m.value != null) {
      ...
      lines.push(`${m.name} **${m.value}${m.unit ? ' ' + m.unit : ''}** (${pct(m.change_pct)} ${m.change_basis})`);
    }
  });
  if (!lines.length) return null;
  const text = lines.join('; ') + '. Dated market context for the energy-route chokepoints — not a stated cause.';
...
function weekAnswer(data) {
  ...
  if (ev?.events?.length) {
    ...
    const recent = ev.events.slice(0, 3).map((x) => `${x.entity} (${x.type})`).join(', ');
...
function listDisrupted(data) {
  ...
  if (!active.length) return answer({ text: 'Nothing is flagged right now.', cites: ['flags.json'], kind: 'list' });
  const facts = active.map((f) => F(f.severity, 'flags.json'));
  const text = `**${active.length}** active disruption(s):\n`
    + active.map((f) => `• ${f.entity} — ${f.kind.replaceAll('_', ' ')}, sev ${f.severity} (${pct(f.pct_change)})`).join('\n');
...
const HELP = answer({
  ...
  kind: 'help',
  suggestions: ["What's going on?", 'What is the biggest risk?', 'Tell me about the Strait of Hormuz', 'Am I exposed?'],
});
...
export function ask(q, data, prebuiltIndex) {
  ...
  // exposure
  if (has('exposed', 'exposure', 'at risk', 'my trade', 'my lanes', 'hitting me', 'affect me', 'impact me'))
    return exposure(e, data) || HELP;

  // trend / momentum
  if (has('improv', 'recover', 'getting worse', 'worsen', 'deteriorat', 'easing', 'trend', 'momentum', 'better or worse'))
    return trending(data) || summarize(data);

  // this week / changes
  if (has('this week', 'changed', 'what is new', 'whats new', 'new flag', 'latest', 'recently', 'since last'))
    return weekAnswer(data) || summarize(data);

  // biggest risk / worst
  if (has('biggest', 'worst', 'most critical', 'most severe', 'top risk', 'should i worry', 'main risk', 'highest'))
    return biggestRisk(data);

  // list
  if (has('which', 'list', 'all disrupt', 'what is disrupt', 'whats disrupt', 'how many', 'everything'))
