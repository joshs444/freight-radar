...
import { routeLane, REROUTE_DELAY, DEFAULT_CHOKE_DELAY } from './routing.ts';
import type { Resolver, RouteDetail } from './routing.ts';
import type { CsvLane } from './csv.ts';
import type {
  Flag,
  CostBand,
  ExposedLane,
...
type BandKey = 'low' | 'expected' | 'high';

interface RoutedLane extends CsvLane {
  _route_cps: Set<string>;
  _routing: RouteDetail;
  _origin_portid: string | number | null;
  _dest_portid: string | number | null;
}

type FlagWithBusiness = Flag & { business: BusinessImpact };

const CARRYING_RATE = [0.2, 0.25, 0.3];
const REROUTE_PREMIUM_PER_TEU_DAY = [15, 25, 40];
const KEYS: BandKey[] = ['low', 'expected', 'high'];

// A faithful CostBand from the per-key entries the band builders produce.
const toBand = (o: Record<string, number>): CostBand => ({
  low: o.low,
  expected: o.expected,
  high: o.high,
});
...
function delayBand(flag: Flag): CostBand {
  const d = delayDays(flag);
  return { low: Math.max(1, pyRound(d * 0.7)), expected: d, high: pyRound(d * 1.35) };
}

const carryingBand = (value: number, db: CostBand): CostBand =>
  toBand(
...
const sumBand = (...bands: CostBand[]): CostBand =>
  toBand(
    Object.fromEntries(KEYS.map((k) => [k, pyRound(bands.reduce((s, b) => s + (b[k] || 0), 0))]))
  );

function prepareRoutes(lanes: RoutedLane[], resolver: Resolver): void {
...
function businessForFlag(flag: Flag, lanes: RoutedLane[]): BusinessImpact {
...
export function computeExposure(
  ...
  rawLanes: CsvLane[],
  resolver: Resolver
): { flags: FlagWithBusiness[]; summary: ExposureSummary } {
  // prepareRoutes mutates each lane in place, adding the _route_* fields that make it
  // a RoutedLane — so the raw CSV lanes the caller passes become routed here.
  const lanes = rawLanes as RoutedLane[];
  prepareRoutes(lanes, resolver);
  const out = flags.map((f): FlagWithBusiness => ({ ...f, business: businessForFlag(f, lanes) }));
  const active = out.filter((f) => f.lifecycle !== 'resolved');
  const carry: CostBand = { low: 0, expected: 0, high: 0 };
  const wc: CostBand = { low: 0, expected: 0, high: 0 };
  const total: CostBand = { low: 0, expected: 0, high: 0 };
  const exposedLaneIds = new Set<string>();
  let disrupted = 0;
  for (const f of active) {
    ...
    if (b.lane_count) {
      disrupted += 1;
      KEYS.forEach((k) => {
        carry[k] += b.carrying_cost_of_delay_usd?.[k] ?? 0;
        wc[k] += b.working_capital_tied_up_usd?.[k] ?? 0;
        total[k] += b.total_cost_of_disruption_usd?.[k] ?? 0;
      });
