...
import type { World, WorldMetric } from '../types.ts';

// `invert` is emitted only on the "disrupted" tile (narrative/world.py) and is not
// part of the canonical WorldMetric shape, so widen it locally without redefining.
type WorldMetricWithInvert = WorldMetric & { invert?: boolean };
...
function Trend({ pct, dir, invert }: TrendProps) {
...
export default function WorldRibbon({ world }: WorldRibbonProps) {
  ...
  return (
    <div className="fr-world" title={`Global ocean-freight activity · ${world.source}`}>
      ...
      <div className="fr-world-tiles">
        {world.metrics.map((m: WorldMetricWithInvert) => (
