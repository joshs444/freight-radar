...
export default function StressDetail({ stress, onClose, onPickEntity }: StressDetailProps) {
  ...
  const dirWord =
    stress.wow_direction === 'up' ? 'up' : stress.wow_direction === 'down' ? 'down' : 'flat';
    ...
    >
      ...
      >
        ...
        <div className="fr-sd-head">
          <div>
            ...
            <div className="fr-sd-score" style={{ color: lv.color }}>
              {stress.index}
              <span>/100</span>
              <em style={{ color: lv.color, background: lv.tint, borderColor: lv.edge }}>
                {stress.label}
              </em>
            </div>
            <div className="fr-sd-wow">
              {dirWord === 'flat' ? 'flat' : `${dirWord} ${Math.abs(wow)} pts`} vs last week
            </div>
          </div>
          <div className="fr-sd-spark">
            <Sparkline
              values={(stress.history || []).slice(-90)}
              width={170}
              height={46}
              color={lv.color}
              strokeWidth={1.6}
        ...
        <p className="fr-sd-what">
          A single 0–100 score we compute from IMF PortWatch vessel data to summarise how disrupted
          global ocean freight is right now versus normal. It's{' '}
          <b>our own composite — not an official index</b>, and it's fully transparent. Here's
          exactly what goes into it.
        </p>

        <div className="fr-sd-grid">
          <div className="fr-sd-comp">
            <div className="fr-sd-comp-h">
              Breadth <b>{stress.breadth}</b>
            </div>
            <p>
              How broadly the network is stressed — an economic-weighted average of every
              chokepoint's deviation from its own normal throughput.{' '}
              <b>
                {stress.chokepoints_disrupted} of {stress.chokepoints_total}
              </b>{' '}
              are disrupted right now.
            </p>
          </div>
          <div className="fr-sd-comp">
            <div className="fr-sd-comp-h">
              Depth <b>{stress.depth}</b>
            </div>
            <p>
              How bad the single <i>worst</i> chokepoint is. Blended in at 40% on purpose, so a
              crisis at one strategic strait (a Suez or a Hormuz) isn't averaged away by 27 calm
              ones.
            </p>
          </div>
        </div>
              ...
              <span className="fr-sd-c-top">
                ...
                <span className="fr-sd-c-num">
                  ...
                  <b style={{ color: c.pct_vs_normal < 0 ? '#c0392b' : '#b07b1e' }}>
                    {' '}
                    ({c.pct_vs_normal > 0 ? '+' : ''}
                    {Math.round(c.pct_vs_normal)}%)
                  </b>
                </span>
              </span>
              <span className="fr-sd-c-bar">
                <i style={{ width: `${(c.contribution / maxC) * 100}%`, background: lv.color }} />
              </span>
            </button>
          ))}
          {!contribs.length && (
            <div className="fr-sd-none">
              No chokepoint is meaningfully off its normal right now.
            </div>
          )}
        </div>
        ...
        {(stress.fastest_deteriorating || stress.most_improved) && (
          <div className="fr-sd-movers">
            {stress.fastest_deteriorating && (
              <span>
                ↗ deteriorating fastest: <b>{stress.fastest_deteriorating.name}</b>
              </span>
            )}
            {stress.most_improved && (
              <span>
                ↘ most improved: <b>{stress.most_improved.name}</b>
              </span>
            )}
          </div>
        )}
        ...
        <div className="fr-sd-scale">
          {SCALE.map((s) => (
            <span key={s.label} className={s.label === stress.label ? 'on' : ''}>
              {s.label} <i>{s.range}</i>
            </span>
          ))}
        </div>
        <div className="fr-sd-foot">
          Source: {stress.source} · as of {stress.as_of}. Click any driver to see it on the globe.
        </div>
      </div>
    </div>
  );
