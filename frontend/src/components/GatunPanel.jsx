...
export default function GatunPanel({ gatun }) {
  ...
  return (
    <div className="fr-gat">
      <div className="fr-gat-head">Panama draft · leading indicator <span>· ACP, not PortWatch</span></div>
      <div className="fr-gat-row">
        ...
        <div className="fr-gat-stat">
          <b className={down ? 'lo' : ''}>{gatun.change_30d_ft > 0 ? '+' : ''}{gatun.change_30d_ft} ft</b>
      ...
      {gatun.level_spark?.length > 1 && (
        <div className="fr-gat-spark"><Sparkline values={gatun.level_spark} width={300} height={26} color="#2d7d9a" /> <span>120d</span></div>
      )}
      <div className={`fr-gat-draft ${draftCut > 0 ? 'cut' : ''}`}>
        Projected max draft <b>{gatun.min_projected_neopanamax_draft_ft} ft</b> Neopanamax
        {draftCut > 0
          ? <> — a <b>{draftCut.toFixed(1)} ft restriction</b> vs the {gatun.normal_max_draft_ft} ft norm{gatun.surcharge_pct_now ? `, ${gatun.surcharge_pct_now}% surcharge` : ''}</>
          : <> — unrestricted (full {gatun.normal_max_draft_ft} ft)</>}
      </div>
      <div className="fr-gat-foot">{gatun.source} · as of {gatun.as_of} · {gatun.disclaimer}</div>
    </div>
  );
}
