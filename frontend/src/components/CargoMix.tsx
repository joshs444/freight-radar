...
import type { CargoMix as CargoMixData } from '../types.ts';

const CARGO_ORDER: (keyof CargoMixData)[] = [
  'container',
  'tanker',
  'dry_bulk',
  'general_cargo',
...
const CARGO_LABEL: Record<keyof CargoMixData, string> = {
  container: 'Container',
  tanker: 'Tanker',
  dry_bulk: 'Dry bulk',
...
const CARGO_COLOR: Record<keyof CargoMixData, string> = {
  container: '#3a6ea5',
  tanker: '#c2611f',
  dry_bulk: '#8a6d3b',
...
interface CargoMixProps {
  mix: CargoMixData | null | undefined;
  unit?: string;
  avgSize?: number | null;
  tonnage?: number | null;
}

export default function CargoMix({ mix, unit = 'transits', avgSize, tonnage }: CargoMixProps) {
  if (!mix) return null;
