...
import type { Disruptions } from '../types.ts';
...
const ALERT: Record<string, string> = { RED: '#c0392b', ORANGE: '#c2611f', GREEN: '#3f7a5a' };
...
export default function HazardsPanel({ disruptions, onPickEntity }: HazardsPanelProps) {
  ...
  return (
    <section className="fr-haz">
      <button className="fr-haz-top" onClick={() => setOpen((o) => !o)}>
        ...
        <span className="fr-haz-count">
          {events.length} recent · {red} red {open ? '▾' : '▸'}
        </span>
      </button>
      {open && (
        <div className="fr-haz-body">
          {events.slice(0, 8).map((e) => {
            ...
            return (
              <div key={e.eventid} className="fr-haz-evt">
                <span className="fr-haz-ico" style={{ color: ALERT[e.alertlevel] }}>
                  {ICON[e.type] || '⚠'}
                </span>
                <div className="fr-haz-main">
                  <div className="fr-haz-name">
                    <span className="fr-haz-alert" style={{ background: ALERT[e.alertlevel] }}>
                      {e.alertlevel}
                    </span>
                    {e.name}
                  </div>
                  <div className="fr-haz-meta">
                    ...
                    {e.n_affected_ports > 0 &&
                      ` · ${e.n_affected_ports} port${e.n_affected_ports > 1 ? 's' : ''}`}
                    {choke && (
                      <>
                        {' '}
                        · near{' '}
                        <button
                          className="fr-haz-link"
                          onClick={(ev) => {
                            ev.stopPropagation();
                            onPickEntity?.(choke.portid);
                          }}
                        >
                          {choke.name}
                        </button>
          ...
          <div className="fr-haz-foot">
            {disruptions?.source} · trailing {disruptions?.window_days}d
          </div>
        </div>
      )}
    </section>
