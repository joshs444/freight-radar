...
export default function StressGauge({ stress, onOpen }: StressGaugeProps) {
  ...
  return (
    ...
    >
      <div className="fr-stress-left">
        <span className="fr-stress-label">
          Ocean Freight Stress <span className="fr-stress-info">ⓘ what's this</span>
        </span>
        <span className="fr-stress-sub">
          {stress.chokepoints_disrupted}/{stress.chokepoints_total} chokepoints disrupted
        </span>
        <div className="fr-stress-bar">
          <i style={{ width: `${pct}%`, background: lv.color }} />
        </div>
      </div>
      <div className="fr-stress-num" style={{ color: lv.color }}>
        {stress.index}
        <span className="fr-stress-max">/100</span>
      </div>
      <span
        className="fr-stress-badge"
        style={{ color: lv.color, background: lv.tint, borderColor: lv.edge }}
      >
        {stress.label}
      </span>
      <span
        className="fr-stress-wow"
        style={{
          color: dir === 'up' ? '#c0392b' : dir === 'down' ? '#0d9488' : 'var(--ink-faint)',
        }}
      >
        {arrow} {wow > 0 ? '+' : ''}
        {wow}
      ...
      <span className="fr-stress-spark">
        <Sparkline values={stress.spark30} color={lv.color} width={76} height={24} />
      </span>
    </button>
  );
}
