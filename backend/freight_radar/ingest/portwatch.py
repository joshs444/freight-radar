...
def _to_frame(rows: list[dict], colmap: dict[str, str]) -> pd.DataFrame:
...
async def load_chokepoint_daily(
    ...
    rows = await client.query_date_window(
        SERVICES["daily_chokepoints"], start, end, chunk_days=45
    )
    return upsert_df(con, "fct_chokepoint_daily", _to_frame(rows, _CHOKE_MAP))


async def load_port_daily(con, client: ArcGISClient, start: date, end: date) -> int:
    rows = await client.query_date_window(
        SERVICES["daily_ports"], start, end, chunk_days=20
    )
    return upsert_df(con, "fct_port_daily", _to_frame(rows, _PORT_MAP))
