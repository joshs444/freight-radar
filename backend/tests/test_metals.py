"""P3 — the metals/bulk-energy SIGNAL honesty four-pack + FDR."""
...
from freight_radar import metals as M
...
def test_registered_as_a_signal_sidecar() -> None:
    entry = next((e for e in ENRICHERS if e[0] == "metals"), None)
    assert entry is not None and entry[2] is False
    assert "metals" in publish._SIDECARS
    assert by_id("metals").kind.value == "SIGNAL"
    assert by_id("metals").metric


def test_copy_carries_no_causal_or_forecast_verb() -> None:
    hits = scan_causal(inspect.getsource(M))
    assert not hits, f"causal/forecast verb in metals copy: {hits}"


def test_basket_enrolls_in_fdr() -> None:
    series = {
        "PIORECRUSDM": _series([100, 101, 99, 100, 102, 98, 100, 101, 99, 100, 101, 99, 200]),
        "PALUMUSDM": _series([100, 101, 99, 100, 102, 98, 100, 101, 99, 100, 101, 99, 100.5]),
    }
    sig = M.compute_signal(series, q=0.10)
    assert sig["counts"]["tested"] == 2
    by = {r["id"]: r for r in sig["items"]}
    assert by["PIORECRUSDM"]["fdr_significant"] is True
    assert by["PALUMUSDM"]["fdr_significant"] is False
...
def test_degrades_to_absent_offline(tmp_path, monkeypatch) -> None:
    ...
    assert not (tmp_path / "metals.json").exists()
