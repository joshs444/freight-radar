"""P3 — the slack SIGNAL honesty four-pack + FDR."""
...
from freight_radar import slack as S
...
def test_registered_as_a_signal_sidecar() -> None:
    entry = next((e for e in ENRICHERS if e[0] == "slack"), None)
    assert entry is not None and entry[2] is False
    assert "slack" in publish._SIDECARS
    assert by_id("slack").kind.value == "SIGNAL"
    assert by_id("slack").metric


def test_copy_carries_no_causal_or_forecast_verb() -> None:
    hits = scan_causal(inspect.getsource(S))
    assert not hits, f"causal/forecast verb in slack copy: {hits}"


def test_basket_enrolls_in_fdr() -> None:
    series = {
        "ISRATIO": _series([100, 101, 99, 100, 102, 98, 100, 101, 99, 100, 101, 99, 200]),
        "RETAILIRSA": _series([100, 101, 99, 100, 102, 98, 100, 101, 99, 100, 101, 99, 100.5]),
    }
    sig = S.compute_signal(series, q=0.10)
    assert sig["counts"]["tested"] == 2
    by = {r["id"]: r for r in sig["items"]}
    assert by["ISRATIO"]["fdr_significant"] is True
    assert by["RETAILIRSA"]["fdr_significant"] is False
