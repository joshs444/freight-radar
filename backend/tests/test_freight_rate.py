"""P3 — the freight transport-cost SIGNAL honesty four-pack + FDR."""

from __future__ import annotations

import inspect

import freight_radar.publish as publish
from freight_radar import _http
from freight_radar import freight_rate as F
from freight_radar.enrich import ENRICHERS
from freight_radar.honesty.lexicon import scan as scan_causal
from freight_radar.registry.layers import EnrichCtx, by_id


def _series(vals: list[float]) -> list[tuple[str, float]]:
    return [(f"20{20 + i // 12:02d}-{i % 12 + 1:02d}-01", v) for i, v in enumerate(vals)]


def test_registered_as_a_signal_sidecar() -> None:
    entry = next((e for e in ENRICHERS if e[0] == "freight_rate"), None)
    assert entry is not None and entry[2] is False
    assert "freight_rate" in publish._SIDECARS
    assert by_id("freight_rate").kind.value == "SIGNAL"
    assert by_id("freight_rate").metric


def test_copy_carries_no_causal_or_forecast_verb() -> None:
    hits = scan_causal(inspect.getsource(F))
    assert not hits, f"causal/forecast verb in freight_rate copy: {hits}"


def test_basket_enrolls_in_fdr() -> None:
    series = {
        "PCU484121484121": _series([100, 101, 99, 100, 102, 98, 100, 101, 99, 100, 101, 99, 200]),
        "PCU482111482111": _series([100, 101, 99, 100, 102, 98, 100, 101, 99, 100, 101, 99, 100.5]),
    }
    sig = F.compute_signal(series, q=0.10)
    assert sig["counts"]["tested"] == 2
    by = {r["id"]: r for r in sig["items"]}
    assert by["PCU484121484121"]["fdr_significant"] is True
    assert by["PCU482111482111"]["fdr_significant"] is False


def test_degrades_to_absent_offline(tmp_path, monkeypatch) -> None:
    def boom(*_a, **_k):
        raise RuntimeError("network blocked")

    monkeypatch.setattr(_http, "client", boom)
    monkeypatch.setattr(_http, "get", boom)
    ctx = EnrichCtx(
        db_path=tmp_path / "x",
        out_dir=tmp_path,
        flags_path=tmp_path / "f.json",
        as_of="2026-05-31",
        today="2026-06-06",
    )
    assert "error" in F.run(ctx)
    assert not (tmp_path / "freight_rate.json").exists()
