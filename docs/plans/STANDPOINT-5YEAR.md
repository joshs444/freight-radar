...
> **Status: strategy satellite of [STANDPOINT-VISION.md](STANDPOINT-VISION.md)
> (reconciled 2026-06-09).** **Precedence:** this doc governs *strategy* — depth over
> breadth, the hard layer cap, the retirement policy, the revenue fences; the VISION's
> P4–P5 breadth lists are a capacity-bounded menu under this plan's cap. Year-1 (H1)
> items shipped 2026-06-06/07: the `.git`-bloat fix (`876e276`), `SPINE == 1`
> (`d747370`), the rendered DERIVED language firewall (`9845c86`), graceful-rot
> self-demotion (`1449094`), the `verify()` honest-no demo (`40b836a`). Still open:
> matrix-sharding the weekly Action.

## 1. Executive Summary

Standpoint is a deployed flagship whose entire value is that **its honesty is a build artifact, not a promise** — and the single most important truth in this plan is that *the credibility is already banked; nothing here depends on future revenue*. The defensible asset is not a market position but a reference artifact: epistemic restraint made a compile error, machine-enforced across a 2065-port measured spine, a typed registry, an import-graph firewall, an FDR gate, and a bitemporal store. The one bet that compounds *both* that banked credibility *and* any future product optionality, at near-zero marginal founder-cost, is **depth on the substrate: bitemporal-everywhere + the git-bloat fix + a rendering/language honesty firewall** — plumbing that makes the single-maintainer story true while keeping every "company" door open as a clean seam rather than a sunk build. Everything that requires *selling, supporting, or SLA-ing* is fenced behind falsifiable revenue gates, because the only resource that is actually scarce is one founder's hours, and those hours are mostly committed elsewhere. This plan optimizes for surviving the maintainer's *success* — the real competitor is not Kpler, it's "the maintainer's attention moves on and the feeds quietly rot" — so the architecture's north star is a system that **rots gracefully and self-defends**, not one that scales a sales motion that doesn't exist.
...
**What this plan IS:**
- A strategic operating doc for a *deployed reference project*, live at
  `joshs444.github.io/freight-radar`. **Reconciliation note (2026-06-09):** an earlier
  draft claimed "the tactical build (P0–P6) is genuinely done, verified, and live" against
  a commit ref that predates the `.git` history purge (`876e276`) and no longer resolves —
  both removed. The true state: VISION **P0–P3 shipped + verified**; most of **P4–P6's
  headline surfaces shipped early** (⌘K palette, lenses, Source Ledger, drift detector,
  Nearby, comparison matrix, the gated DERIVED reasoner + `hyp_*` tier); the **P4–P5
  breadth menu, Action matrix-sharding, and the P5 exposure rasters remain unbuilt** —
  see the per-phase stamps in [STANDPOINT-VISION.md](STANDPOINT-VISION.md) §5.
- Optimized first for **credibility** — the proven, banked value — and second for keeping product/company optionality alive as *seams*, not roadmaps.
- Explicit about its single binding constraint: **one founder's hours.** A single founder can *ship artifacts and write posts*. The moment a motion requires selling, supporting, or SLA-ing, it competes with those committed hours and loses.
...
| **The flagship demos** | (a) `verify(claim)` as a *demo*, not a service: a screen-recording of an agent suppressing an ungrounded geopolitics claim because the store returned "no measured observation supports this." (b) The honesty harness published MIT-OSS with one blog post. | One screen-recording + one OSS repo + "I made honesty a compile error" post live | — |
...
```
Gate 0 — REFERENCE ARTIFACT ......................... DONE. Already paid for itself.
  Value: banked credibility as EVIDENCE one maintainer can build this, today,
  with zero customers. Pricing it as a product LOWERS this value (now it must
  survive a market that pays for restraint — which nobody does) while banking
  nothing.
        │
        ▼
Gate 1 — SIDE PROJECT (0–6 mo) .......... THE ONLY GATE COMMITTED TODAY
...
**What would have to be TRUE for this to be a company, not a reference project:**
...
3. The PIT track record is *allowed to accumulate* for 3+ years without the maintainer abandoning it — which collides directly with the hours committed elsewhere.

**The honest call:** pursue Gate 1, hold the line at Gate 2's *honesty-clamped* revenue test, treat Gate 3 as real-but-conditional. **The most likely failure mode isn't competition — it's the maintainer's attention moving on and the feeds rotting.** Build for that world, because it's the likely one.
...
> **working demonstration → reference implementation → cited standard.**
> Year 1 *demonstrates* (one maintainer built the honest thing alone). Years 2–3 *reference* (others build against it). Years 4–5 *standardize* (the honesty predicates are what people cite when they ask "is this source trustworthy?").
...
| 2 | **Honesty-as-compile-fact** (firewall, FDR, drift contracts, golden masters) | **Medium** | The trust *accelerant* and the credibility proof — but copyable in a quarter, so NOT the durable moat. |
...
**So the corrected moat thesis:** the firewall is the **day-one trust accelerant and the credibility proof**, not the year-five defense. The year-five defense is **time**: the appreciating bitemporal corpus (#1) and — most durably — **the banked, certain credibility value**, which no satellite competes with at all.
...
- **The quant-buyer fantasy** — the critique is right and it's load-bearing: **no quant puts an un-indemnified, coverage-gapped hobby feed in a money-moving model.** PIT correctness is *necessary and nowhere near sufficient*. Concede that the open-data ceiling means **there is likely no paying data customer** — and that's fine, because the value was already banked as credibility, not revenue.
...
| **Design-partner consulting** | **FENCE: inbound-only** | commodity/macro desks | never go find them | A 30-hr/week sales-and-delivery motion that competes head-on with the maintainer's committed hours. It's a *second job*, not a complement. Only if a desk *inbounds*. And the consulting trap is real: a desk *will* ask for the forecast — saying no costs revenue, saying yes costs the brand, so the Gate 2 honesty clause governs it. |

**The honest bottom line on money:** the proven, certain return is **the credibility the harness has already banked as a reference artifact**. Everything else is a flagged bet on a credibility market converting to a budget market, which is unproven.
...
**The 80/20 — the few bets that compound BOTH banked credibility AND product optionality, at near-zero marginal founder-cost (do ONLY these):**
1. **Bitemporal-everywhere + git-bloat fix + graceful-rot metabolism.** The thing that *actually fails first*; makes the single-maintainer story true; zero buyers required. Pure credibility AND product foundation.
2. **The `verify()` "honest no" *demo*** (screen-recording of an agent suppressing an ungrounded claim) — the most legible single artifact in the plan. The demo, not the endpoint.
3. **`SPINE == 1` CI predicate + the DERIVED language firewall** — hours of work, closes the one honest gap, and a perfect case study.
4. **Harness as MIT-OSS + one blog post** ("I made honesty a compile error") — a backlink magnet at near-zero cost.

**Explicitly DO NOT do:**
- The outward overclaim leaderboard / referee position (brand-corrosive *and* a credibility liability).
...
**The real failure mode (the only competitor that wins):** the maintainer's attention moves on and the feeds quietly rot. The entire plan is optimized around *surviving that success* — which is why graceful-rot, self-demotion, and a fixed capacity cap outrank every move against billion-dollar incumbents.
...
**Weeks 6–10 — Build the two flagship artifacts.**
...
**Weeks 10–13 — Convert artifacts into distribution + run the Gate 1 test.**
- Publish the harness post and the `verify()` video; point the project's public write-ups at them alongside the existing flagship.
...
- Confirm the metabolism: let the refresh run untended for the back half of the quarter and verify it survives. That receipt — *a 100-layer-capable platform that maintains itself* — is the single-maintainer story, proven, not asserted.

**The 90-day through-line:** none of this needs a second human, a dollar of metered API, or a customer. It hardens the substrate, closes every honest gap, and produces two artifacts that move the *certain* win — banked credibility — while keeping every product door open as a clean seam. That is the whole bet: **build for the world where the maintainer's attention moves elsewhere, because that's the likely one, and make Standpoint the most defensible reference artifact in the field on the way there.**
